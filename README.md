...
Sanitized public release projection only. It is not the canonical working database and never receives private seed drafts.
...
**This repository may contain:** Governance now; later only gated, sanitized release projections produced by the authorized publishing pipeline.

**This repository must not contain:** Raw seed drafts, source copies, private data, prompts/completions, reviewer information, exact actionable fragments, application code, secrets, or credentials.
...
```bash
git clone https://github.com/urologai/urologai-content.git
cd urologai-content
git fetch --all --prune
git switch main
...
- [open issues](https://github.com/urologai/urologai-content/issues);
- [open pull requests](https://github.com/urologai/urologai-content/pulls);
- remote branches; and
...
No content-generation block is permitted here yet. B02 must first be completed in `urologai-web`, followed by the plan’s platform, provenance, licensing, release-gate, and publishing prerequisites.

Do not manually populate this repository with background articles. Interactive seed work remains private and individually batched until the export and publication gates exist.
